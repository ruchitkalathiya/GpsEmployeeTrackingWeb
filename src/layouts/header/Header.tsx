...
const Header: React.FC = () => {
    ...
    return (
        <>
            <div>
                <Navbar collapseOnSelect expand="lg" className={location.pathname === "/" ? 'header' : 'white-header'}>
                    <Container fluid>
                        <Navbar.Brand className="homepagelogo"><Link to="/" className={location.pathname === "/" ? 'd-block' : 'd-none'}><h2 className="text-white">EmployeeGPS</h2></Link></Navbar.Brand>
                        <Navbar.Brand className="mainlogo"><Link to="/" className={location.pathname === "/" ? 'd-none' : 'd-block'}><Image src="./img/Group.png" /></Link></Navbar.Brand>
                        <Navbar.Toggle aria-controls="responsive-navbar-nav" />
                        <Navbar.Collapse id="responsive-navbar-nav ">
                            <Nav className="ml-auto menu-link">
                                {/* <DropdownButton title={t('Select_Language')} onSelect={function (evt) { i18next.changeLanguage() }} className={location.pathname === "/" ? 'select-btn' : 'select-btn select-btn-two'}>
                                    <Dropdown.Item>
                                ...
                                <Buttons
                                    onClick={() => { return setsignuppopup(true); }}
                                    children={t('logIn.Log_In')}
                                    ButtonStyle={location.pathname === "/" ? 'button-border ml-md-4 mr-auto' : 'button-border-two ml-md-4 mr-auto'}
                                /> */}
                            </Nav>
                        </Navbar.Collapse>
                    </Container>
